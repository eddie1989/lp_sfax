...
task :check_fax_response => :environment do
  fax_requests_queue_ids = FaxRecord.where("send_fax_queue_id is null and SendFaxQueueId is not null and (max_fax_response_check_tries is null OR max_fax_response_check_tries < #{MAX_FAX_RESPONSE_CHECK_TRIES})").pluck(:SendFaxQueueId)
...
task :sendback_final_response_to_client => :environment do
  ...
  if array_of_jsons.blank?
    ...
    p "the array of jsons till Time #{Time.now} is  "
    p array_of_jsons
  end
end
